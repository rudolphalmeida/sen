...
class Bus {
   ...
   public:
    ...
    Bus(std::shared_ptr<Cartridge> cartridge, std::shared_ptr<Ppu> ppu, std::shared_ptr<Apu> apu, std::shared_ptr<Controller> controller)
        : cartridge{std::move(cartridge)}, internal_ram(0x800, 0xFF), ppu{std::move(ppu)}, apu{std::move(apu)}, controller{std::move(controller)} {
